...
class _PostDonationScreenState extends State<PostDonationScreen> {
  ...
  // Match AI recommendation categories
  ...
  Future<void> _handleAIDetection() async {
    ...
    try {
      // Run AI detection immediately
      final detectedLabel = await AIService.detectItemLabel(_imageFile!);
      final detectedColor = await AIService.detectColor(_imageFile!);

      if (mounted) {
        // Get AI-recommended category
        final recommendedCategory =
            AIService.recommendDonationCategory(detectedLabel);

        setState(() {
          _isDetecting = false;
          _aiRecommendedCategory = recommendedCategory;
          // Auto-fill category if user hasn't changed it from default
        ...
        if (detectedLabel != null) detectedInfo.add('Label: $detectedLabel');
          ...
          SnackBar(
            content: Text(
              detectedInfo.isEmpty
                  ...
                  : 'AI detected: ${detectedInfo.join(", ")}. Recommended category: $recommendedCategory',
            ),
